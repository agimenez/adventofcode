...
func (p *Program) SetMem(addr, val int) {
	...
	utils.Dbg(4, "  (SetMem) p.mem[%d] = %d", addr, val)
	p.mem[addr] = val
}
...
func (p *Program) getMem(addr int) int {
	...
	utils.Dbg(4, "  (get) p.mem[%d] = %d", addr, p.mem[addr])
	return p.mem[addr]
}
func (p *Program) Run(input <-chan int, output chan<- int) {
	...
	for op := p.mem[p.pc]; op != 99; {
		utils.Dbg(5, "MEM = %v", p.mem)
		utils.Dbg(3, "pc = %d; op = %v", p.pc, op)

		opcode := op % 100
		utils.Dbg(3, "opcode = %v", opcode)

		switch opcode {
		case 1: // Add
			utils.Dbg(3, " INSTR = %v", p.mem[p.pc:p.pc+4])
			a, b, c := p.fetchParameter(1), p.fetchParameter(2), p.getAddrIndex(3)
			utils.Dbg(3, " ADD %d %d -> %d", a, b, c)
			p.SetMem(c, a+b)
			p.pc += 4
		case 2: // Mul
			utils.Dbg(3, " INSTR = %v", p.mem[p.pc:p.pc+4])
			a, b, c := p.fetchParameter(1), p.fetchParameter(2), p.getAddrIndex(3)
			utils.Dbg(3, " MUL %d %d -> %d", a, b, c)
			p.SetMem(c, a*b)
			p.pc += 4
		case 3: // In
			utils.Dbg(3, " INSTR = %v", p.mem[p.pc:p.pc+2])
			var in, dst int
			in, dst = <-input, p.getAddrIndex(1)
			utils.Dbg(3, " IN  %d -> mem[%d]", in, dst)
			p.SetMem(dst, in)
			p.pc += 2
		case 4: // Out
			utils.Dbg(3, " INSTR = %v", p.mem[p.pc:p.pc+2])
			src := p.fetchParameter(1)
			utils.Dbg(3, " OUT %d", src)

			output <- src
			p.pc += 2

		case 5: // JMP IF TRUE
			utils.Dbg(3, " INSTR = %v", p.mem[p.pc:p.pc+3])
			tst, newpc := p.fetchParameter(1), p.fetchParameter(2)
			utils.Dbg(3, " JMP %d if %d != 0", newpc, tst)
			if tst != 0 {
				p.pc = newpc
			} else {
				p.pc += 3
		...
		case 7: // LT
			...
			utils.Dbg(3, " LT %d %d %d", first, second, dst)
			if first < second {
				p.SetMem(dst, 1)
			} else {
				p.SetMem(dst, 0)
		...
		case 8: // EQ
			utils.Dbg(3, " INSTR = %v", p.mem[p.pc:p.pc+4])
			first, second, dst := p.fetchParameter(1), p.fetchParameter(2), p.getAddrIndex(3)
			utils.Dbg(3, " EQ %d %d %d", first, second, dst)
			if first == second {
				p.SetMem(dst, 1)
			} else {
				p.SetMem(dst, 0)
...
func (p *Program) fetchParameter(n int) int {
	mode := p.instructionMode(n)
	parameter := p.mem[p.pc+n]
	utils.Dbg(4, "   param[%d](%d) mode: %d, base %d, memsize %d", n, parameter, mode, p.base, len(p.mem))

	if mode == 0 {
		// position mode
		val := p.getMem(parameter)
		utils.Dbg(4, "   (posmode) -> mem[%d] = %d", parameter, val)
		return val
	} else if mode == 2 {
		// relative mode
		val := p.getMem(p.base + parameter)
		utils.Dbg(4, "   (relmode) -> mem[%d+%d] = %d", p.base, parameter, val)
		return val
	}

	// immediate mode
	utils.Dbg(4, "   (immmode) -> = %d", parameter)
	return parameter
}
