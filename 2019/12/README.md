# [Advent of Code](/)
  ...
  * [[Settings]](/2019/settings)
  * [[Log Out]](/2019/auth/logout)
...
#   0.0.0.0:[2019](/2019)
  ...
  * [[Leaderboard]](/2019/leaderboard)
  * [[Stats]](/2019/stats)

Our [sponsors](/2019/sponsors) help make Advent of Code possible:

[Educative.io](http://educative.io/AdventOfCode) \- Gain in-demand tech
skills, at the speed you want. Text-based courses with live coding
environments help you learn without the fluff.
...
extreme [radiation](https://en.wikipedia.org/wiki/Magnetosphere_of_Jupiter),
and a [whole lot of
moons](https://en.wikipedia.org/wiki/Moons_of_Jupiter#List) swirling around.
You decide to start by tracking the four largest moons: *Io* , *Europa* ,
...
and `z`), the velocity of each moon changes by *exactly +1 or -1* to pull the
moons together. For example, if Ganymede has an `x` position of `3`, and
Callisto has a `x` position of `5`, then Ganymede's `x` velocity *changes by
+1* (because `5 > 3`) and Callisto's `x` velocity *changes by -1* (because `3
< 5`). However, if the positions on a given axis are the same, the velocity on
...
energy*. A moon's *potential energy* is the sum of the [absolute
values](https://en.wikipedia.org/wiki/Absolute_value) of its `x`, `y`, and `z`
position coordinates. A moon's *kinetic energy* is the sum of the absolute
values of its velocity coordinates. Below, each line shows the calculations
for a moon's potential energy (`pot`), kinetic energy (`kin`), and total
energy:
...
Your puzzle answer was `344724687853944`.

Both parts of this puzzle are complete! They provide two gold stars: **

At this point, you should [return to your Advent calendar](/2019) and try
another puzzle.
